...
const $ = {
  ...
  getWeatherImage(code, isNight) {
    return WEATHER_IMAGE_PERFIXER + '/' + path.join(isNight ? 'night' : 'day') + '/' + path.join(`${code}.svg`)
  },
  getBackgroundImage(weather, isNight) {
    return BACKGROUND_PERFIXER + '/' + path.join(isNight ? 'night' : 'day') + '/' + path.join(`${weather}.jpg`)
  },
 ...
 */
  ...
  _now: (data, _data) => {
    ...
    let hours = new Date().getUTCHours() + 8  //UTC时间,比如我们是东八区，UTC时间是我们的时区时间减8个小时
    if (hours > 24) {
      hours -= 24
    }
